...
- When annotation-derived `.rmsx.in` targets are unavailable, `rmsx_runner.py` attempts fallback extraction from `rsmviewer/database/user_annotations/RNAMotifScanX/PDB_prebuild.tgz` (if present) before aborting.
