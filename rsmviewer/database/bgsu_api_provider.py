"""
RSMViewer - BGSU RNA 3D Hub Provider
Fetches motif data from the BGSU RNA 3D Hub API.

Loop data: BGSU API at https://rna.bgsu.edu/rna3dhub/loops/download/{PDB_ID}
  - Provides the residue-level loop data.

Semantic annotations: BGSU API at https://rna.bgsu.edu/rna3dhub/pdb/{PDB_ID}/motifs
  - Provides motif family labels (K-turn, GNRA, C-loop, etc.)
...
class BGSUAPIProvider(BaseProvider):
    ...
    def get_motifs_for_pdb(self, pdb_id: str) -> Dict[str, List[MotifInstance]]:
        ...
        try:
            ...
            # Try to fetch semantic annotations from the BGSU API
            annotations = None
            try:
                annotations = self._fetch_html_annotations(pdb_id)
                if annotations:
                    print(f"Successfully fetched {len(annotations)} annotations from the BGSU API for {pdb_id}")
            except Exception as e:
                print(f"BGSU API annotation fetch failed for {pdb_id}: {e}")
            ...
            if annotations:
                ...
                _lg.warning(
                    ...
                    f"the BGSU RNA 3D Hub API for {pdb_id}. "
    ...
    def _fetch_html_annotations(self, pdb_id: str) -> Optional[Dict[str, Dict]]:
        """
        Fetch semantic annotations from the BGSU RNA 3D Hub API.
        ...
        try:
            ...
            with urllib.request.urlopen(request, timeout=self.REQUEST_TIMEOUT, context=ssl_context) as response:
                if response.status == 200:
                    ...
                    if parser.annotations:
                        ...
                        print(f"No annotations found via the BGSU API for {pdb_id}")
                        return None
                else:
            ...
            if e.code == 404:
                print(f"PDB {pdb_id} not found via the BGSU API")
            else:
                print(f"HTTP error fetching from the BGSU API for {pdb_id}: {e.code} {e.reason}")
            return None
            
        except urllib.error.URLError as e:
            print(f"Network error fetching from the BGSU API for {pdb_id}: {e.reason}")
            return None
            
        except Exception as e:
            print(f"Error parsing the BGSU API response for {pdb_id}: {e}")
            return None
