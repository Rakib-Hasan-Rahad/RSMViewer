...
SOURCE_ID_MAP: Dict[int, Dict] = {
    ...
    5: {
        ...
        'tool': 'fr3d',
        'description': 'FR3D local pipeline — extracts Hairpin Loops (HL) and Internal Loops (IL) from any PDB using the RNA 3D Motif Atlas flankSS algorithm (Petrov et al. 2013). Requires fr3d-python. Configure once with a JSON config file; rmv_load_motif auto-runs the pipeline when results are not yet cached.',
        'coverage': 'All HL and IL loops (Hairpin + Internal); any PDB with CIF available',
        'mode': 'user',
        'command': 'rmv_db 5',
    },
    6: {
    ...
    8: {
        ...
        'category': 'USER ANNOTATIONS',
        'tool': 'nobias',
        'description': 'NoBIAS analysis output (custom user files)',
        'coverage': 'Custom uploads',
        'mode': 'user',
        'supports_filtering': True,
        'command': 'rmv_db 8',
        'command_with_filtering': 'rmv_db 8 [on|off]',
        'command_with_custom_pvalues': 'rmv_db 8 MOTIF_NAME p_value ...',
    },
}
