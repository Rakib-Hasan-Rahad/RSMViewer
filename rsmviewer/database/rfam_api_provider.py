...
class RfamAPIProvider(BaseProvider):
    ...
    def _get_parsed_instances_for_motif(
        ...
        # Rfam's website relaunched; the old /motif/{id}/alignment?acc=...&format=stockholm
        # query-param form now 404s. The current form is /motif/{id}/alignment/stockholm,
        # and it 406s if an explicit 'Accept: text/plain' header is sent - omit it.
        url = f"{self.API_BASE_URL}/motif/{rfam_motif_id}/alignment/stockholm"
