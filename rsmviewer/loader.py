...
class VisualizationManager:
    ...
    def _apply_view_color(self, selection: str,
        ...
        try:
            if isinstance(color_spec, (tuple, list)):
                rgb = tuple(float(v) for v in color_spec)
                color_name = f"_rmv_view_color_{abs(hash(rgb))}"
                self.cmd.set_color(color_name, rgb)
                self.cmd.color(color_name, selection)
                return
            ...
            if not color_text:
                return
            if ',' in color_text:
                rgb_parts = [float(p.strip()) for p in color_text.split(',') if p.strip()]
                if len(rgb_parts) == 3:
                    color_name = f"_rmv_view_color_{abs(hash(tuple(rgb_parts)))}"
                    self.cmd.set_color(color_name, tuple(rgb_parts))
                    self.cmd.color(color_name, selection)
                    return

            self.cmd.color(color_text, selection)
        except Exception as exc:
            self.logger.warning(f"Could not apply view color '{color_spec}': {exc}")
    ...
    def view_motif_type(self, motif_type: str,
        ...
        unless an explicit override color is supplied for this view.
        """
        motif_type = motif_type.upper().strip()
        loaded_motifs = self.motif_loader.get_loaded_motifs()
        ...
        for detail in motif_details:
            ...
            for chain, resi_list in chain_residues.items():
                ...
                if sel:
                    ...
                    self._apply_view_color(instance_sel, color_spec)
